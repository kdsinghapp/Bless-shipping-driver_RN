  ...
  try {
    ...
    if (isSuccessWithData || is404WithUserAndToken) {
      ...
      param.navigation.reset({
        index: 0,
        routes: [{ name: ScreenNameEnum.VEHICLE_SELECTION }],
      });
      return parsed;
    }
...
/** Get driver tasks - GET /driver/tasks?tab=new */
const GetDriverTasksApi = async (
  tab: string,
  setLoading: (loading: boolean) => void,
  onUnauthorized?: () => void,
): Promise<any[]> => {
  try {
    ...
    const url = `${base_url}/${endpoints.driverTasks}?tab=${tab}`;
    console.log('Fetching Driver Tasks:', url);
    ...
    console.error('Get Driver Tasks API Error:', error);
    return [];
  } finally {
    setLoading(false);
  }
...
/** Assign vehicle to driver - POST /vehicle/assign/driver */
const AssignVehicleToDriverApi = async (
  vehicleId: string,
  setLoading: (loading: boolean) => void,
): Promise<any> => {
  try {
    ...
    const response = await fetch(`${base_url}/${endpoints.assignVehicle}`, requestOptions);
    const text = await response.text();
    let parsed = JSON.parse(text);

    if (parsed?.success) {
      successToast(parsed?.message || 'Vehicle assigned successfully');
      return parsed;
    } else {
      errorToast(parsed?.message || 'Failed to assign vehicle');
      return undefined;
    }
  } catch (error) {
